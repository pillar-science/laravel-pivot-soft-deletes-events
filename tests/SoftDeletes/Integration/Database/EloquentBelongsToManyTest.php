...
class EloquentBelongsToManyTest extends DatabaseTestCase
{
    ...
    public function testCustomPivotClass()
    {
        ...
        $this->assertSame('2017-10-10 10:10:10', $post->tagsWithCustomPivot[0]->pivot->getAttributes()['created_at']);
    ...
    public function testFirstOrFailMethod()
    {
        ...
        $post->tags()->where('id', 10)->firstOrFail();
    }
    ...
    {
        $this->expectException(ModelNotFoundException::class);
        $this->expectExceptionMessage(sprintf('No query results for model [%s] 10', Tag::class));
    ...
    {
        $this->expectException(ModelNotFoundException::class);
        $this->expectExceptionMessage(sprintf('No query results for model [%s] 10, 11', Tag::class));
    ...
    {
        $this->markTestIncomplete('The original test seems to try to mismatch columns and it fails.');
